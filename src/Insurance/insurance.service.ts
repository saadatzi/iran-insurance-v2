...
export class InsuranceService {
  ...
  async thirdParty(
    filterInsuranceDTO: FilterInsuranceDTO,
    isPreview: boolean,
  ): Promise<JSON[]> {
    /*
    ...
    try {
      ...
      const thisCarInformation = await this.VehicleModel.findOne({
        _id: carModel,
      })
        .select('-createdAt -updatedAt -__v -_id')
        .populate({
          path: 'brand',
          model: this.VehicleBrand,
          select: { __v: 0, types: 0, createdAt: 0, updatedAt: 0, _id: 0 },
        })
        .populate({
          path: 'type',
        ...
        .populate({
          path: 'detail',
          model: this.VehicleDetail,
          select: { __v: 0, types: 0, createdAt: 0, updatedAt: 0, _id: 0 },
          populate: {
            path: 'anotherTypeUnit',
            model: this.VehicleUnit,
            select: { __v: 0, types: 0, createdAt: 0, updatedAt: 0, _id: 0 },
          },
        })
        .exec();
      ...
      if (OP.hasInsurance === true && OP.isNewCar === false) {
        ...
        const hasInsuranceData = {
          ...
          damageDiscountPercentage:
            filterInsuranceDTO.damageDiscountPercentage + 5 || 5,
          // درصد تخفیف راننده // دقیقا مثل تخفیف ثالث
          driverDiscountPercentage:
            filterInsuranceDTO.driverDiscountPercentage + 5 || 5,
          // سال ساخت خودرو - برای محاسبه سن خودرو
          builtYear: filterInsuranceDTO.builtYear || '2004',
          // قیمت خسارت راننده (همون عددی که همزمان با قیمت خسارت راننده تغییر میکنه)
          driverDamagePrice: thisCarInformation.detail.driverDamagePrice,
          ...
          previousInsuranceStartDate:
            filterInsuranceDTO.previousInsuranceStartDate,
          ...
          previousInsuranceNumber:
            filterInsuranceDTO.previousInsuranceNumber || null,
        };
        // چک میکنیم که آیا خسارت گرفته قبلا یا نه
        if (hasDamageReceived === true) {
          // تعداد دفعات دریافت خسارت مالی
          hasInsuranceData['propertyDamageCount'] =
            filterInsuranceDTO.propertyDamageCount;
          // تعداد ذفعات دریافت خسار جانی
          hasInsuranceData['casualtyDamageCount'] =
            filterInsuranceDTO.casualtyDamageCount;
          // تعداد دفعات دریافت خسارت حوادث راننده
          hasInsuranceData['driverAccidentDamageCount'] =
            filterInsuranceDTO.driverAccidentDamageCount;
        }
        prices = await this.IC.thirdPartyHasInsuranceCalculator(
          hasInsuranceData,
        );
      }
      ...
      throw new BadRequestException('prices in insurance service');
    }
  }
}
